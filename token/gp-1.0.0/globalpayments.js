	...
	***************************************************************************** */
	...
	var actionOnload = (function () {
	    ...
	    var data = {
	        ...
	        ORDER_ID: orderId,
	        PAYER_EXIST: options.customerExists || "0",
	        TIMESTAMP: timestamp,
	        VALIDATE_CARD_ONLY: options.validateOnly || "1"
	    };
	    if (options.payerExists) {
	        data.PAYER_REF = options.customerReference; // opt config
	        data.PMT_REF = options.paymentReference; // opt config
	    }
	...
	var assetBaseUrl = "https://hps.github.io/token/gp-1.0.0/";
	        ...
	        gateway.actions
	            ...
	            if (data["card-holder"]) {
	                ...
	                resp.details.cardholderName =
	                    resp.details.cardholderName || data["card-holder"];
	            }
	            if (data["card-track"]) {
	...
	var actionAccumulateDataAndTokenize = (function (id, type, data) {
	    ...
	    if (JSON.stringify(w.dataFields.sort()) ===
	        ...
	        tokenize({
	            ...
	            "card-holder": w.dataContents["card-holder"] !== undefined &&
	                w.dataContents["card-holder"],
	...
	var actionPaymentRequestStart = (function (id, data) { return __awaiter(_this$5, void 0, void 0, function () {
	    var request, response, e_1, code, token, d, cardNumber, bin, last4, type, e_2;
	    return __generator(this, function (_a) {
	        switch (_a.label) {
	            case 0:
	            ...
	            case 4:
	                ...
	                return [4 /*yield*/, tokenize({
	                        ...
	                        "card-holder": response.details.cardholderName || "",
	...
	var IframeField = /** @class */ (function (_super) {
	    ...
	    function IframeField(type, selector, src) {
	        ...
	        if (!_this.container) {
	            bus.emit("error", new Error("IframeField: target cannot be found with given selector"));
	            return _this;
	        }
	...
	function start (selector, details, instruments, options) {
	    return new Promise(function (resolve, reject) {
	...
	var reset = function () {
	    var el = document.getElementById(iframeHolderId);
	    if (!el) {
	        return;
	    }
	    if (el.remove) {
	...
	function setup$1 (selector, details, instruments, options) {
	    return new Promise(function (resolve, reject) {
	        var el = document.querySelector(selector);
	        if (!el) {
	            return reject({
	        ...
	        Events.addHandler(el, "click", function (e) {
	            e.preventDefault();
